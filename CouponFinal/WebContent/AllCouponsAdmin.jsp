...
<body>
	<form action="/CouponFinal/ServletDemo/DelCoupon" method="POST">
		<%
			...
			<fieldset data-role="controlgroup" data-mini="true">
				...
				<label>  <img src="/CouponFinal/Images/<%=tmp.getImage()%>" alt='image'
						width="42" height="42"> <input type="radio" name="radios" value=<%=id%>
					checked="checked" > <%
 	} else {
 %> <label>  <img src="/CouponFinal/Images/<%=tmp.getImage()%>" alt='image'
						width="42" height="42"> <input type="radio" name="radios" value=<%=id%> >
 		...
 		while (couponList.hasNext()) {
			...
			tmp = couponsHibernate.getCouponFromList(couponList);
 			if (id == 1) {
 %> <label>  <img src="/CouponFinal/Images/<%=tmp.getImage()%>" alt='image'
							width="42" height="42"><input type="radio" name="radios"
 ...
 %>
			...
			<input type="submit" name="delCoupon" value="Del Coupon"> </input>
			<button type="button" name="main_menu"
				onclick="window.location.href='/CouponFinal/ServletDemo/AdminMenu'">Go Back to
				Menu</button>
